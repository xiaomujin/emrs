...
use emrs_infra::scanner::Scanner;
use emrs_infra::stores::media_store;
...
impl ProbeStage {
    ...
    pub async fn run_pending(&self, batch_size: i64) -> (usize, usize, usize) {
        let rows = media_store::list_pending_file_sources(&self.db, batch_size).await;
        if rows.is_empty() {
            return (0, 0, 0);
        }
        ...
        for chunk in rows.chunks(self.concurrency) {
            ...
            for row in chunk {
                ...
                let remote_path = row.remote_path.clone();
                ...
                set.spawn(async move {
                    let Some(p) = path.or(remote_path) else {
                        ...
                        media_store::mark_source_failed(&db, ms_id).await;
                        return (ms_id, false);
                    };
