...
/// [`image_ids_batch`] 的行类型（裁定 C10 命名化）。
...
pub async fn image_ids_batch(db: &Db, relation_ids: &[i64]) -> Result<HashMap<i64, ImageTypeIds>> {
    ...
    let mut q = sqlx::query_as::<_, ImageIdRow>(&sql);
    ...
    for row in rows {
        let e = ids.entry(row.parent_id).or_default();
        match row.image_type.to_ascii_lowercase().as_str() {
            // 单图类型取 id 最小首行（已按 id 升序，首个即最小）；Backdrop 全量收集。
            "primary" => {
                e.primary.get_or_insert(row.image_id);
            }
            "backdrop" => e.backdrops.push(row.image_id),
            "logo" => {
                e.logo.get_or_insert(row.image_id);
            }
            "thumb" => {
                e.thumb.get_or_insert(row.image_id);
            }
            "banner" => {
                e.banner.get_or_insert(row.image_id);
            }
            _ => {}
        }
    }
