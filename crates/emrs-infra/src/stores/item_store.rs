...
/// [`claim_pending_scrape`] 的行类型：待刮削条目 + 当前尝试次数（裁定 C10 命名化）。
#[derive(Debug, Clone, sqlx::FromRow)]
pub struct PendingScrapeRow {
    pub id: i64,
    pub title: String,
    /// DB 小写类型（movie/series）。
...
pub async fn claim_pending_scrape(db: &Db, batch: i64) -> Vec<PendingScrapeRow> {
    sqlx::query_as::<_, PendingScrapeRow>(
        "SELECT id, title, type AS item_type, date_air, tmdb_id, \
         COALESCE(scrape_attempts, 0) AS attempts FROM item \
...
// ---------------------------------------------------------------------------
// 删除检测的级联清理（SQL 自 pipeline.rs::delete_item 内联语句原样收归）。
// ---------------------------------------------------------------------------

/// 查询直属子项 id（删除检测：父删则子项逐层清理；失败按空处理，与迁移前
/// `unwrap_or_default()` 一致）。
pub async fn child_ids(db: &Db, item_id: i64) -> Vec<i64> {
    sqlx::query_scalar("SELECT id FROM item WHERE parent_id = ?")
        .bind(item_id)
        .fetch_all(db.pool())
        .await
        .unwrap_or_default()
...
/// 物理删除 item 及其关联行（分类/人员/制片/标签关联、图片、用户进度）。
/// 虚拟条目不动（`is_virtual = 0` 条件）。调用方保证该 item 已无剩余
/// media_source；子项清理由调用方逐层调 [`child_ids`] + 本函数。
pub async fn delete_item_cascade(db: &Db, item_id: i64) {
...
/// [`fetch_parents_by_ids`] 的行类型（裁定 C10 命名化）。
#[derive(sqlx::FromRow)]
struct ParentRow {
    id: i64,
    title: String,
...
async fn fetch_parents_by_ids(db: &Db, ids: &[i64]) -> Result<HashMap<i64, (String, Option<i64>)>> {
    ...
    let mut q = sqlx::query_as::<_, ParentRow>(&sql);
    for id in ids {
        q = q.bind(id);
    }
    let rows = q.fetch_all(db.pool()).await.context("parents by id")?;
    for row in rows {
        out.insert(row.id, (row.title, row.parent_id));
    }
    Ok(out)
}
...
async fn fetch_media_batch(db: &Db, item_ids: &[i64]) -> Result<HashMap<i64, MediaCols>> {
    ...
    let mut q = sqlx::query_as::<_, MediaBatchRow>(&sql);
    for id in item_ids {
        q = q.bind(id);
    }
    ...
    for r in rows {
        ...
        out.entry(r.item_id).or_insert(MediaCols {
            container: r.container,
            file_second: r.file_duration,
            uuid: r.uuid,
            name: r.name,
...
/// [`fetch_media_batch`] 的行类型（裁定 C10 命名化；列别名与迁移前 SQL 一致）。
#[derive(sqlx::FromRow)]
struct MediaBatchRow {
    item_id: i64,
    container: Option<String>,
    file_duration: Option<i64>,
    uuid: Option<String>,
    name: Option<String>,
...
/// [`count_episodes_batch`] 的行类型（裁定 C10 命名化；COUNT 列加别名供 FromRow 映射）。
#[derive(sqlx::FromRow)]
struct EpisodeCountRow {
    parent_id: i64,
    total: i64,
    unplayed: i64,
}
...
async fn count_episodes_batch(
    ...
    let sql = if series_mode {
        format!(
            "SELECT season.parent_id, COUNT(*) AS total, \
                    COUNT(CASE WHEN uid.id IS NULL OR uid.played = 0 THEN 1 END) AS unplayed \
             FROM item ep \
             JOIN item season ON season.id = ep.parent_id \
             LEFT JOIN user_item_data uid ON uid.item_id = ep.id AND uid.user_id = ? \
        ...
        format!(
            "SELECT ep.parent_id, COUNT(*) AS total, \
                    COUNT(CASE WHEN uid.id IS NULL OR uid.played = 0 THEN 1 END) AS unplayed \
             FROM item ep \
             LEFT JOIN user_item_data uid ON uid.item_id = ep.id AND uid.user_id = ? \
             WHERE ep.type = 'episode' \
    ...
    let mut q = sqlx::query_as::<_, EpisodeCountRow>(&sql).bind(user_id);
    ...
    for row in rows {
        out.insert(row.parent_id, (row.total, row.unplayed, None));
    }
    Ok(())
}
