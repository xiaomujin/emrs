...
/// genre / studio 批取的行类型（`(item_id, 规范行 id, name)`，裁定 C10 命名化）。
#[derive(sqlx::FromRow)]
struct ItemNameRow {
    item_id: i64,
    id: i64,
...
pub async fn taxonomy_batch(
    ...
    let mut gq = sqlx::query_as::<_, ItemNameRow>(&genre_sql);
    for id in item_ids {
        gq = gq.bind(id);
    }
    for row in gq
    ...
    let mut pq = sqlx::query_as::<_, ItemPersonRow>(&people_sql);
    for id in item_ids {
        pq = pq.bind(id);
    }
    let people_rows: Vec<ItemPersonRow> = pq
        .fetch_all(db.pool())
        .await
        .context("query item people batch")?;

    // 收集所有 people_id，批量查头像图片行 id（每人取 id 最小一张）
    let all_people_ids: HashSet<i64> = people_rows.iter().map(|r| r.id).collect();
    ...
    for row in people_rows {
        out.entry(row.item_id)
            ...
            .push(PersonBrief {
                ...
                primary_image_id: people_image_id.get(&row.id).copied(),
            });
    }
    ...
    let mut sq = sqlx::query_as::<_, ItemNameRow>(&studio_sql);
    for id in item_ids {
        sq = sq.bind(id);
    }
    for row in sq
...
/// [`item_counts`] 的行类型（裁定 C10 命名化；标量子查询加别名供 FromRow 映射）。
...
pub async fn item_counts(db: &Db) -> Result<ItemCountsRow> {
    let row = sqlx::query_as::<_, ItemCountsRow>(
        "SELECT \
            (SELECT COUNT(*) FROM item WHERE type = 'movie') AS movies, \
            (SELECT COUNT(*) FROM item WHERE type = 'series') AS series, \
            (SELECT COUNT(*) FROM item WHERE type = 'episode') AS episodes",
    )
    .fetch_one(db.pool())
    .await
