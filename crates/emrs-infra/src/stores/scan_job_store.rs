...
/// [`pending`] 的行类型（裁定 C10 命名化）。
#[derive(Debug, Clone, sqlx::FromRow)]
pub struct PendingScanJobRow {
    pub id: i64,
    pub library_id: i64,
...
pub async fn pending(db: &Db) -> Vec<PendingScanJobRow> {
    sqlx::query_as::<_, PendingScanJobRow>(
...
/// [`poll_status_batch`] 的行类型（裁定 C10 命名化；COALESCE 列加别名）。
#[derive(Debug, Clone, sqlx::FromRow)]
...
/// 批量读取指定任务的进度（status + 计数），供 admin 轮询。
/// `added_items` / `updated_items` 为 NULL 时取 0（与旧口径一致）。
pub async fn poll_status_batch(db: &Db, ids: &[i64]) -> Vec<JobProgressRow> {
    if ids.is_empty() {
        return Vec::new();
    }
    let ph = vec!["?"; ids.len()].join(",");
    let sql = format!(
        "SELECT status, COALESCE(added_items, 0) AS added, COALESCE(updated_items, 0) AS updated \
         FROM scan_job WHERE id IN ({ph})"
    );
    let mut q = sqlx::query_as::<_, JobProgressRow>(&sql);
    for id in ids {
        q = q.bind(id);
    }
...
pub async fn cancel_pending_batch(db: &Db, ids: &[i64]) {
    ...
    let mut q = sqlx::query(&sql).bind(emrs_core::emby::format_time_now());
    for id in ids {
        q = q.bind(id);
    }
