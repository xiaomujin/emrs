...
impl Scanner {
    ...
    pub(super) fn tmdb_config(&self) -> crate::tmdb::TmdbConfig {
        crate::tmdb::TmdbConfig {
    ...
    pub async fn probe_media_source(&self, media_source_id: i64, file_path: &str) -> bool {
        ...
        let (media, probe_ok, reason) = match probed {
            Ok(m) => (m, true, None),
            Err(reason) => (crate::probe::ProbeMedia::default(), false, Some(reason)),
        };
    ...
    async fn process_video_file(
        ...
        let parsed = crate::strm::StrmPath {
    ...
    async fn process_episode(
        ...
        if pn.episode_end > pn.episode {
            let now_v = emrs_core::emby::format_time_now();
    ...
    pub async fn attach_backdrops(
        &self,
        item_id: i64,
        backdrops: &[crate::tmdb::TmdbImage],
        max_count: usize,
    ) {
        image_store::delete_images_of_type(&self.db, "item", item_id, "backdrop").await;
        let mut sorted: Vec<&crate::tmdb::TmdbImage> = backdrops.iter().collect();
        sorted.sort_by(|a, b| {
            b.vote_average
                .unwrap_or(0.0)
