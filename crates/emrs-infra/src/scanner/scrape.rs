...
/// [`Scanner::upsert_people`] 查询已有行用的行类型（裁定 C10 命名化，替代原 4 元组别名）。
#[derive(Debug, sqlx::FromRow)]
struct PeopleExistingRow {
    id: i64,
...
impl Scanner {
    ...
    /// （按 tmdb_id 幂等，重复关联跳过）。
    async fn link_studios(&self, item_id: i64, companies: &[crate::tmdb::TmdbCompany]) {
        let now = emrs_core::emby::format_time_now();
        let mut sort_order: i64 = 0;
        for c in companies {
    ...
    /// （按 tmdb_id 幂等，重复关联跳过）。
    async fn link_tags(&self, item_id: i64, keywords: Option<&crate::tmdb::KeywordResp>) {
    ...
    async fn upsert_people(
        ...
        let existing: Option<PeopleExistingRow> = sqlx::query_as(
        ...
        if let Some(row) = existing {
            ...
            let needs_enrich = row
                .description
                .as_deref()
                .is_none_or(|s| s.trim().is_empty())
                && row.birthday.is_none()
                && row.deathday.is_none();
            return (row.id, needs_enrich);
        }
        let _ = sqlx::query(
